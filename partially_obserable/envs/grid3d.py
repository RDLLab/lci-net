...
class Grid3D:
    ...
    def __init__(self, x_size, y_size, z_size, obs_dirs, obstacle_prob, transition_prob,
            ...
            for o_down in [self.FREE_SPACE, self.OBSTACLE]:
                for o_left in [self.FREE_SPACE, self.OBSTACLE]:
                    for o_up in [self.FREE_SPACE, self.OBSTACLE]:
                        for o_north in [self.FREE_SPACE, self.OBSTACLE]:
                            for o_south in [self.FREE_SPACE, self.OBSTACLE]:
                                observations.append([o_south, o_north, o_up, o_left, o_down, o_right])
    ...
    def __get_obs(self, z, y, x):
        ...
        for dz, dy, dx in self.obs_dirs:
            if random.random() < self.obs_success_prob:
                # correct observation
                o = self.grid[z + dz][y + dy][x + dx]
            else:
                # incorrect observation
                o = 1 - self.grid[z + dz][y + dy][x + dx]
            obs.append(o)
        return np.array(obs)
