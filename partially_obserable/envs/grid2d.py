...
class Grid2D:
    """
    Class representing a parameterised 2D grid world environment.
    ...
    def __init__(self, x_size, y_size, obs_dirs, obstacle_prob, transition_prob,
    ...
    def reset(self):
        ...
        while len(partition_list) > 1:
            ...
            for y1, x1 in p1:
                # check above
                if (y1 - 2, x1) in p2 and (y1 - 2, x1) not in boundary:
                    boundary.append((y1 - 1, x1))
                # check below
                if (y1 + 2, x1) in p2 and (y1 + 2, x1) not in boundary:
                    boundary.append((y1 + 1, x1))
                # check left
                if (y1, x1 - 2) in p2 and (y1, x1 - 2) not in boundary:
